...
public class SellAuthLoginInterceptor extends HandlerInterceptorAdapter{
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
       ...
       // login처리를 담당하는 사용자 정보를 담고 있는 객체를 가져옴
        ...
        Object obj2 = session.getAttribute("seller1");
        ...
        if ( obj == null ){
        	...
        	response.setContentType("text/html; charset=UTF-8");
        	PrintWriter out = response.getWriter();
        	out.println("<script>alert('판매자 전용페이지 입니다.'); location.href='/ex/';</script>");
            //response.sendRedirect("/ex/user/login");
        }
